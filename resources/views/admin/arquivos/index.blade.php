...
@section('content')
    <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                @foreach($grupos as $index => $grupo)
                    <div class="group relative bg-[#1e1e1e] rounded-lg border border-gray-800 hover:border-[#f2c700] transition-all duration-300 p-6 cursor-pointer transform hover:scale-[1.02] hover:shadow-lg hover:shadow-[#f2c700]/20 stagger-animation card-hover"
                        ...
                        onclick="window.location='{{ route('admin.arquivos.grupo', $grupo) }}'" role="button" tabindex="0"
                        ...
                        <div class="mb-4 transform group-hover:scale-110 transition-transform duration-300">
                            <svg class="w-16 h-16 text-[#f2c700] group-hover:text-[#d9b300] transition-colors" fill="currentColor"
                                viewBox="0 0 24 24">
                                <path
                        ...
                        <div
                            class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300 flex gap-2 transform translate-y-2 group-hover:translate-y-0">
                            <button
                                onclick="event.stopPropagation(); editGrupo({{ $grupo->id }}, '{{ addslashes($grupo->nome) }}', '{{ addslashes($grupo->descricao ?? '') }}')"
                            ...
                            <button
                                onclick="event.stopPropagation(); deleteGrupo({{ $grupo->id }}, '{{ addslashes($grupo->nome) }}')"
        ...
        @else
            <div class="text-center py-16 bg-[#1e1e1e] rounded-lg border border-gray-800">
                ...
                <p class="mt-2 text-sm text-gray-400 max-w-md mx-auto">Comece criando um grupo para organizar seus arquivos e
    ...
    <!-- Modal: Criar Grupo -->
    <div id="createGrupoModal" x-data="{ open: false }" x-show="open" x-cloak @keydown.escape.window="open = false"
        ...
        <div @click.away="open = false"
            ...
            <form action="{{ route('admin.arquivos.grupos.store') }}" method="POST">
                ...
                <div class="mt-6 flex gap-3">
                    <button type="button" onclick="closeModal('createGrupoModal')"
    ...
    <!-- Modal: Deletar Grupo -->
    <div id="deleteGrupoModal" x-data="{ open: false }" x-show="open" x-cloak @keydown.escape.window="open = false"
        class="fixed inset-0 bg-black bg-opacity-75 z-50 flex items-center justify-center p-4" style="display: none;"
        ...
        <div class="bg-[#1e1e1e] rounded-lg p-6 w-full max-w-md border border-red-800/50 transform transition-all"
            ...
            <p class="text-white mb-2">Tem certeza que deseja deletar o grupo <strong class="text-red-400"
                    id="delete_grupo_nome"></strong>?</p>
